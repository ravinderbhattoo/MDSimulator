...
function plot_energy_(res::DiffEqBase.ODESolution; E="PE")
...
function plot_energy(res::DiffEqBase.ODESolution; E=["PE", "KE", "TE"])
    KE, temp, PE = summary_(res)
    TE = KE + PE
    N = length(KE)
    t = res.t[1]:res.t[end]/(N-1):res.t[end]
...
function plot_temperature(res::DiffEqBase.ODESolution)
    KE, temp, PE = summary_(res)
    N = length(KE)
...
function plot_momentum(res::DiffEqBase.ODESolution)
