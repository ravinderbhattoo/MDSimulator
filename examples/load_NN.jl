...
using BSON: @load
@load "./output/trained_NN.bson" model
...
# scatter!(r2, [potential_energy(i, lj_params) for i in r2], markersize=1)
# scatter!(r2, [potential_energy(i, lj_params) for i in r2], color="red", markersize=1)
scatter!(rx2, rdata2*std_.+m_, color="green", markersize=2)
scatter!(rx, rdata*std_.+m_, color="yellow", markersize=2)
vline!([0.75σ_], lw=2)
#
